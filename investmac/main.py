...
app = FastAPI(title="InvestMAC API", version="7.0.0")
...
def color_status(metric, value):
    # devuelve: good | bad | neutral + explicación
    if metric == "P/E":
        if value is None: return "neutral", "Sin dato."
        if value < 10: return "good", "Valuación baja (potencial oportunidad)."
        if value > 35: return "bad", "Valuación exigente (riesgo de corrección)."
        return "neutral", "Valuación intermedia."
    if metric == "RSI14":
        if value > 70: return "bad", "Sobrecompra: posible pausa/corrección."
        if value < 30: return "good", "Sobreventa: posible rebote."
        return "neutral", "Zona media sin extremo."
    if metric == "Retorno 1Y":
        if value > 15: return "good", "Momentum positivo."
        if value < -10: return "bad", "Debilidad relevante."
        return "neutral", "Comportamiento moderado."
    if metric == "Volatilidad":
        if value > 45: return "bad", "Riesgo alto por oscilación."
        if value < 20: return "good", "Riesgo de oscilación menor."
        return "neutral", "Riesgo medio."
    return "neutral", "Sin regla específica."
...
def analyze(ticker: str, horizon: str = "mediano"):
    ...
    rsi_status, rsi_why = color_status("RSI14", s1["rsi14"])
    pe_status, pe_why = color_status("P/E", pe)
    ret_status, ret_why = color_status("Retorno 1Y", s1["retorno_1y_pct"])
    vol_status, vol_why = color_status("Volatilidad", s1["volatilidad_pct"])
    ...
    return {
        ...
        "scores": s1,
        "details": [
            {"item": "P/E", "value": pe, "status": pe_status, "why": pe_why, "action": "Si está rojo, entrar con más cautela."},
            {"item": "RSI14", "value": s1["rsi14"], "status": rsi_status, "why": rsi_why, "action": "Si está extremo, esperar confirmación."},
            {"item": "Retorno 1Y", "value": s1["retorno_1y_pct"], "status": ret_status, "why": ret_why, "action": "Si es débil, reducir riesgo."},
            {"item": "Volatilidad", "value": s1["volatilidad_pct"], "status": vol_status, "why": vol_why, "action": "A más volatilidad, menor tamaño de posición."}
        ],
        "comparativa": {
...
<head>
  ...
  <style>
    ...
    input,select,button{width:100%;padding:10px;border-radius:8px;border:1px solid #ccc;font-size:16px}
...
<div class="wrap">
  <div class="card">
    ...
    <div class="grid2">
      <div>
        <label>Ticker</label>
        <input id="ticker" placeholder="AAPL, GGAL, NVDA..."/>
      </div>
      <div>
  ...
  <div class="card" id="main" style="display:none">
    <div class="grid3">
      ...
      <div><div class="k">Market Cap</div><div id="mcap">-</div></div>
      <div><div class="k">P/E</div><div id="pe">-</div></div>
      <div><div class="k">Dividend Yield %</div><div id="divy">-</div></div>
    </div>
  </div>
  ...
  <div class="card" id="scores" style="display:none">
    <h3>Veredicto</h3>
    <div class="grid3">
      <div><div class="k">Veredicto</div><div class="v neutral" id="vered">-</div></div>
      <div><div class="k">Score Total</div><div class="v" id="total">-</div></div>
      <div><div class="k">RSI14</div><div class="v" id="rsi">-</div></div>
    </div>
  </div>

  <div class="card" id="detailCard" style="display:none">
    <h3>Detalle con semáforo y explicación</h3>
...
async function run(){
  ...
  document.getElementById("mcap").textContent=d.company?.market_cap ?? "-";
  document.getElementById("pe").textContent=d.company?.pe ?? "-";
  document.getElementById("divy").textContent=d.company?.dividend_yield_pct ?? "-";

  document.getElementById("vered").textContent=d.scores?.veredicto ?? "-";
  document.getElementById("total").textContent=d.scores?.total ?? "-";
  document.getElementById("rsi").textContent=d.scores?.rsi14 ?? "-";
  ...
  document.getElementById("detailTable").innerHTML = `
    ...
    <tbody>
      ...
      <tr>
        <td>${x.item}</td>
        <td>${x.value ?? "-"}</td>
        <td class="${cls(x.status)}">${x.status}</td>
        <td>${x.why ?? "-"}</td>
        <td>${x.action ?? "-"}</td>
      </tr>`).join("")}
    </tbody>
  ...
  const ct=d.comparativa[tkr], cp=d.comparativa[pr];
